...
// *** SET THIS TO 1, 2, or 3 before flashing each anchor ***
#define ANCHOR_ID 3

#define RESPONSE_TIMEOUT_MS 50  // Increased from 10 for reliability
...
static long long t_roundB = 0;  // Changed to long long to avoid overflow
static long long t_replyB = 0;  // Changed to long long to avoid overflow
...
public:
  ...
  static void ds_sendRTInfo(long long t_roundB, long long t_replyB);
...
void DWM3000Class::ds_sendFrame(int stage) {
  ...
  bool error = true;
  for (int i = 0; i < 50; i++) {
    if (sentFrameSucc()) { error = false; break; }
  }
  if (error) Serial.println("[ERROR] Could not send frame successfully!");
}

// FIX: t_roundB and t_replyB are now long long
void DWM3000Class::ds_sendRTInfo(long long t_roundB, long long t_replyB) {
  setMode(1);
  write(0x14, 0x01, destination & 0xFF);
  write(0x14, 0x02, sender & 0xFF);
  write(0x14, 0x03, 4);
  write(0x14, 0x04, (uint32_t)(t_roundB & 0xFFFFFFFF));
  write(0x14, 0x08, (uint32_t)(t_replyB & 0xFFFFFFFF));
  setFrameLength(12);
  TXInstantRX();
}
...
int DWM3000Class::sentFrameSucc() {
  int sys_stat = read(GEN_CFG_AES_LOW_REG, 0x44);
  if ((sys_stat & SYS_STATUS_FRAME_TX_SUCC) == SYS_STATUS_FRAME_TX_SUCC) return 1;
  return 0;
}
...
long double DWM3000Class::getClockOffset() {
  if (config[0] == CHANNEL_5) return getRawClockOffset() * CLOCK_OFFSET_CHAN_5_CONSTANT / 1000000;
  else return getRawClockOffset() * CLOCK_OFFSET_CHAN_9_CONSTANT / 1000000;
}

long double DWM3000Class::getClockOffset(int32_t sec_clock_offset) {
  if (config[0] == CHANNEL_5) return sec_clock_offset * CLOCK_OFFSET_CHAN_5_CONSTANT / 1000000;
  else return sec_clock_offset * CLOCK_OFFSET_CHAN_9_CONSTANT / 1000000;
}
...
void DWM3000Class::printRoundTripInformation() {
  ...
  long long rx_ts = readRXTimestamp();
  Serial.print("TX Timestamp: "); Serial.println(tx_ts);
  Serial.print("RX Timestamp: "); Serial.println(rx_ts);
}
...
void DWM3000Class::printDouble(double val, unsigned int precision, bool linebreak) {
  ...
  Serial.print(".");
  unsigned int frac;
  if (val >= 0) frac = (val - int(val)) * precision;
  else frac = (int(val) - val) * precision;
...
void DWM3000Class::setBitLow(int reg_addr, int sub_addr, int shift) { setBit(reg_addr, sub_addr, shift, 0); }
void DWM3000Class::setBitHigh(int reg_addr, int sub_addr, int shift) { setBit(reg_addr, sub_addr, shift, 1); }
  ...
  if (!readWriteBit) {
    ...
    if (dataLen == 0) {
      if (data > 0) {
        ...
        if ((payload_bits % 8) > 0) payload_bytes++;
      } else {
        payload_bytes = 1;
      }
    } else {
...
void setup() {
  Serial.begin(115200);
  delay(2000);  // Give DWM3000 time to boot fully
...
void loop() {
  ...
  // ---- STAGE 0: Wait for Poll from Tag ----
  case 0:
    t_roundB = 0;
    t_replyB = 0;
    ...
    if (rx_status == 1) {
      // FIX: Clear system status BEFORE reading frame fields
      DWM3000.clearSystemStatus();
      ...
      if (DWM3000.getDestinationID() == ANCHOR_ID) {
        ...
        // FIX: Clear status before going back to RX to prevent stale frame re-reads
        DWM3000.clearSystemStatus();
        DWM3000.standardRX();
      }
    } else if (rx_status == 2) {
      Serial.println("[ERROR] RX error in stage 0");
      DWM3000.clearSystemStatus();
      DWM3000.standardRX();
  ...
  // ---- STAGE 1: Poll received — send Response ----
  case 1:
    // FIX: Read RX timestamp BEFORE sending response (captures T2 before TX overwrites state)
    rx = DWM3000.readRXTimestamp();   // T2: time Poll arrived at anchor

    DWM3000.ds_sendFrame(2);          // Send Response (stage 2)

    tx = DWM3000.readTXTimestamp();   // T3: time Response was sent

    t_replyB = tx - rx;               // t_replyB = T3 - T2
    curr_stage = 2;
    last_ranging_time = millis();
    break;

  // ---- STAGE 2: Wait for Final from Tag ----
  case 2:
    rx_status = DWM3000.receivedFrameSucc();
    if (rx_status == 1) {
      retry_count = 0;
      ...
      if (DWM3000.ds_isErrorFrame()) {
        Serial.println("[WARNING] Received error frame in stage 2!");
        curr_stage = 0;
        DWM3000.standardRX();
      } else if (DWM3000.ds_getStage() != 3) {
        Serial.print("[WARNING] Unexpected stage in stage 2: ");
        Serial.println(DWM3000.ds_getStage());
        DWM3000.ds_sendErrorFrame();
        DWM3000.standardRX();
        curr_stage = 0;
      ...
      Serial.println("[ERROR] RX error in stage 2");
      DWM3000.clearSystemStatus();
      curr_stage = 0;
      DWM3000.standardRX();
    } else if (millis() - last_ranging_time > RESPONSE_TIMEOUT_MS) {
      Serial.println("[WARNING] Timeout waiting for Final");
      if (++retry_count > MAX_RETRIES) {
        resetRadio();
        retry_count = 0;
      }
  ...
  // ---- STAGE 3: Final received — send RT Info back to Tag ----
  case 3:
    rx = DWM3000.readRXTimestamp();   // T6: time Final arrived at anchor
    t_roundB = rx - tx;               // t_roundB = T6 - T3 (anchor round trip)

    DWM3000.ds_sendRTInfo(t_roundB, t_replyB);  // Send DATA packet with timing info
  ...
  default:
    Serial.print("[ERROR] Unknown stage (");
    Serial.print(curr_stage);
    Serial.println("). Reverting to stage 0");
    curr_stage = 0;
    DWM3000.standardRX();
    break;
  }
